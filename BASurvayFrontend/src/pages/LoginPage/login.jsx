...
function Login() {
  ...
  const validateEmail = (email) => {
      const re = /^[a-z0-9]((\.|\+)?[a-z0-9]){1,}@bilgeadam(boost)?(akademi)?\.com$/;
    ...
    <div className="loginContent">
      ...
      <img className="vektor7" src={vektor7} alt="vektor7" />
        <div className="user-menu">
          <label id='email' className="username">Kullanıcı Adı:</label> <br />
          <input
      ...
      {error && <p>{error}</p>}
          ...
          <div className="cbottom">
            <button className="button" onClick={handleSubmit}>GİRİŞ</button>
          </div>
        </div>
      </div>
    </div>
